...
def index():
    ...
    stats = {
        ...
        'rev_table': df2[["Category","App","Reviews","Rating"]].groupby(["Category","App"]).\
         ...
         head(5).reset_index().head(10).to_html(
    ...
    cat_order = df2.groupby(['Category']).agg({'App' : 'count'},axis=1).rename({'App': 'Total'}, axis=1).sort_values('Total', ascending = False).head().reset_index()
    ...
    df2[['Android Ver']] = playstore[['Android Ver']].astype('category')
    df3 = df2.groupby(["Android Ver"]).count().sort_values('Reviews', ascending = False).head(5).reset_index()
    X = df3['Android Ver']
    Y = df3['App']
    my_colors = 'rgbkymc'
    # bagian ini digunakan untuk membuat kanvas/figure
